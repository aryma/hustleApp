...
var openFB = (function () {
    ...
    // Default OAuth redirect URL. Can be overriden in init()
        oauthRedirectURL = 'http://condivision.escapenetwork.com/fl_app/customerForm/close.html',

    // Default Cordova OAuth redirect URL. Can be overriden in init()
        cordovaOAuthRedirectURL = "http://condivision.escapenetwork.com/fl_app/customerForm/close.html",
    ...
    function login(callback, options) {
        ...
        function loginWindow_loadStartHandler(event) {
            ...
            if (url.indexOf("access_token=") > 0 || url.indexOf("error=") > 0) {
                ...
                var timeout = 6 - (new Date().getTime() - startTime);
                setTimeout(function () {
                    loginWindow.close();
                }, timeout > 0 ? timeout : 0);
                oauthCallback(url);
        ...
        loginWindow = window.open(loginURL + '?client_id=' + fbAppId + '&redirect_uri=' + redirectURL +
            '&response_type=token&scope=' + scope, '_blank', 'location=yes,clearcache=yes');
