...
/* FB Login */
  function F7_zuckemberg_login () {
   
    FB.api('/me', function(response) {
   
    usr_uid = response.id;
    usr_fbname = response.name;

    localStorage.setItem("usr_uid", usr_uid);
    localStorage.setItem("usr_fbname", usr_fbname);

    userLogin(endpoint,usr_uid,'nopassword',usr_uid);
